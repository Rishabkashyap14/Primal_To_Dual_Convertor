...
void display(string &scope,
			...
			int dualchk)
{
    // cout<<"The LP can be represented as: "<<endl;
    if(dualchk == 0){
	    cout<<endl;
	    cout<<"The objective function is "<<scope<<" z = "<<" ";

	    for(int i = 0; i < vars - 1; i++){
	        cout<<obj[i]<<"x"<<i + 1<<" "<<" + ";
	    }cout<<obj[vars - 1]<<"x"<<vars;
	    ...
	    for(int i = 0; i < cons; i++){
	        ...
	        {
	            cout<<consr[i][j]<<"x"<<j + 1<<" + ";
	        }cout<<consr[i][vars - 1]<<"x"<<vars;
	    ...
	    cout<<"The domains of the varibles (in order) can be represented as: ";
	    for(int i = 0; i < vars - 1; i++){
	        cout<<domain[i]<<",";
	    }cout<<domain[vars - 1];
	    ...
	    cout<<endl;
	    cout<<endl;
	    cout<<"Number of variables = "<<vars;
	    cout<<endl;
	    cout<<"Number of constraints = "<<cons;
	    ...
	    cout<<endl;
		    ...
		    for(int i = 0; i < cons; i++){
		        for(int j = 0; j < vars - 1; j++)
		        {
		            cout<<consr[i][j]<<"p"<<j + 1<<" + ";
		        }cout<<consr[i][vars - 1]<<"p"<<vars;
...
//  	domain = {"R", "R"};
  ...
  domain = {"P", "R"};
    ...
    cout<<"The Original LP as it was given to us: "<<endl;
    display(scope, obj, RHS, consr, p_cons, p_vars, signs, domain, dualchk);
    ...
    canon_primal(p_vars, p_cons, scope, obj, RHS, signs, consr, domain, pc_vars, pc_cons, pc_obj,
            pc_RHS, pc_signs, pc_consr, pc_domain);

    cout<<"The Primal LP in canonical form can be represented as: "<<endl;
    display(scope, pc_obj, pc_RHS, pc_consr, pc_cons, pc_vars, pc_signs, pc_domain, dualchk);
    cout<<endl;


//    string d_scope = "min";
//    int dc_vars = 3, dc_cons = 3;
//    vector<int> dc_obj = {-4, 5, -5};
//    vector<int> dc_RHS = {1, 2, -2};
//    vector<string> dc_signs = {">=", ">=", ">="};
//    vector<vector<int>> dc_consr= {{-2, 3, -3}, {-3, 4, -4}, {3, -4, 4}};
//    vector<string> dc_domain = {"P", "P", "P"};
    ...
    dual_canon(pc_vars, pc_cons, scope, pc_obj, pc_RHS, pc_signs, pc_consr, pc_domain, d_scope,
         dc_vars, dc_cons, dc_obj, dc_RHS, dc_signs, dc_consr, dc_domain);

	dualchk++;
    cout<<"The Dual LP in canonical form can be represented as: "<<endl;
    display(d_scope, dc_obj, dc_RHS, dc_consr, dc_cons, dc_vars, dc_signs, dc_domain, dualchk);
    cout<<endl;
    ...
    general_Dual(dc_vars, dc_cons, d_scope, dc_obj, dc_RHS, dc_signs, dc_consr, dc_domain,
		dg_consr, dg_obj, dg_RHS, dg_domain, dg_signs, dg_vars, dg_cons);
	cout<<"The Final Dual LP in general form can be represented as: "<<endl;
	display(d_scope, dg_obj, dg_RHS, dg_consr, dg_cons, dg_vars, dg_signs, dg_domain, dualchk);
